...
| Recurso | Descrição |
| -----------| ---------- |  
| Alterações na Análise de coorte | Fizemos duas pequenas mudanças no modo como a Análise Cohort, que lançou uma grande atualização em janeiro de 2019, funciona. Essas modificações podem resultar em alterações nos dados retornados.<ul><li>A definição do segmento Dimension de coorte personalizado agora aplica o item de dimensão somente como parte do período de inclusão, não como parte da definição de retorno.</li><li> Para Cálculo de Rolling, aplicávamos os critérios de inclusão em cada um dos períodos de retorno subsequentes. Agora, os critérios de retorno são aplicados a cada período que antecede o período selecionado.</li></ul> |
| Recurso B | Texto |
| Recurso C | Texto |
...
* A definição do segmento Dimension de coorte personalizado agora aplica o item de dimensão somente como parte do período de inclusão, não como parte da definição de retorno.
...
Essa mudança está sendo feita para refletir os padrões do setor em métricas e relatórios e para facilitar a identificação da métrica nos relatórios. O nome da nova métrica ficará imediatamente visível no relatórios do Analytics e nenhuma alteração será necessária em nome do usuário final.
...
AEM 6.1 SP2-CFP18 (6.1.2.18), lançado em 11 de janeiro de 2019, é uma atualização importante que inclui correções essenciais para o cliente lançadas desde a disponibilização geral AEM 6.1 de agosto de 2016.
...
Dicas de solução de problemas para problemas de integração AEM, consulte [Solução de problemas](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/troubleshooting-integration-issues.html)de integração.
