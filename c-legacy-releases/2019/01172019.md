...
last-updated: 28 de janeiro de 2019
...
* [Target](https://marketing.adobe.com/resources/help/en_US/target/rn/target_release_notes.html) - links para a ajuda da solução
...
Notas de versão da [!DNL Experience Cloud] interface, incluindo [!UICONTROL serviços] essenciais da plataforma e administração de produtos.
...
**Observação:** Em março de 2019, a interface da Experience Cloud não oferecerá suporte ao Internet Explorer 11.
...
Melhorias e correções para o [!DNL Experience Cloud] serviço de ID.
...
Esta configuração pode ser acessada por **[!UICONTROL Projeto]** &gt; **[!UICONTROL Informações e configurações do projeto]**.
...
#### [!UICONTROL Aprimoramentos de Canais] de marketing
...
* Correção de um problema com a opção **[!UICONTROL Tentar no Espaço de trabalho]** em [!UICONTROL Reports &amp; Analytics]: os segmentos eram soltos e não apareciam no projeto do Workspace correspondente. (AN-169491)
...
* Administrador: Correção de um problema que impedia que usuários que não fossem administradores pudessem criar regras de processamento, mesmo que lhes tivesse sido atribuída os direitos apropriados. (AN-170690)
...
Na versão 8.1, adicionamos a conformidade da DIL com políticas mais restritas de segurança do Google Tag Manager. Esta versão é disponível somente pelo DTM e como um código independente, não pelo Launch ou pelo AppMeasurement Audience Management Module. As versões futuras incluirão essa alteração. (CORE-24189)

### Correções em [!DNL Audience Manager]
...
Novos recursos, correções e atualizações no [!DNL Experience Manager]. A Adobe recomenda que os clientes com implantações locais implantem os patches mais recentes como um modo de assegurar estabilidade, segurança e desempenho superiores.

### Versões do produto

#### Documentação XML para AEM

A versão 3.2 da solução de Documentação XML foi disponibilizada em 18 de dezembro de 28 18.
...
* Usando Enter no início de um `<li>` ou de um `<p>` elemento quebrado de elemento.
* O codeblock era recuado quando um tópico era editado e o usuário alternava do modo Autor para o modo Origem e vice versa.
* No modo Autor, se a tecla Enter foi usada para adicionar um elemento de parágrafo `<p>` em uma célula de tabela, ela resultava em caracteres de hash extras na saída PDF.
...
Novos recursos da versão [!DNL Adobe Advertising Cloud].
