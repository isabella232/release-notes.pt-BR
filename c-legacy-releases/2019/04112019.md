...
* Adicionado suporte para o Plug-in Audience Manager para IAB TCF, disponível por meio do objeto de aceitação ECID.
...
| Recurso | Descrição |
| -----------| ---------- |  
| Segmentação em contagens distintas | Agora você pode segmentar em uma contagem distinta de itens em uma dimensão. Exemplos: “Visitantes que visualizaram mais de 5 produtos distintos”, ou “Visitas em que mais de 5 páginas distintas foram.” (Observe que a função Contagem distinta aproximada ainda está disponível em Métricas calculadas, se você quiser criar uma métrica que é uma contagem distinta de uma dimensão (por exemplo, número de clientes, número de produtos etc.)) |
...
| Atualizações do Advertising Analytics | Estamos lançando algumas atualizações para a Advertising Analytics: <ul><li>Yahoo Gemini foi absorvido pelo Microsoft Bing em 31 de março. Como consequência, a opção de conta de publicidade do Yahoo Gemini não está mais disponível. </li><li>O Google implementou um novo padrão de rastreamento e, em vez de usar o Modelo _de_ rastreamento, agora usamos o Sufixo _de URL_ final.</li></ul> |
...
* Correção de um problema que ocorria ao tentar alternar segmentos em reportlets de painel. (AN-177056)
* (Relatórios e análises) Corrigido um problema que impedia a seleção de métricas no relatório Visão geral do Canal. (AN-176786)
* (Gerenciador de segmentos) Corrigido um problema que resultava em uma mensagem de erro ao tentar editar um segmento que era compartilhado com várias fontes de dados de Audience Manager. (AN-175353)
...
* (Administrador) Corrigido um problema ao tentar editar eVars para vários conjuntos de relatórios. (AN-168150)
* (Administrador) Corrigido um problema que impedia transferir ativos para outro usuário na interface do usuário herdada do gerenciamento de usuários. (AN-176630, AN-173974)
...
| Visual Experience Composer | O Visual Experience Composer (VEC) inclui as seguintes melhorias para agilizar o seu trabalho e torná-lo mais eficiente: <ul><li>É possível editar o estilo de um elemento, incluindo a imagem de fundo, no VEC. (TGT-15001)</li><li>O público alvo suporta HTML5 usando configurações na v4.5.1 ou superior. (TGT-33618)</li></ul> |

**Aprimoramentos, correções e alterações**
...
* Aprimoramos a renderização de ofertas de imagem no seletor de Ativos. A exibição e seleção de ofertas de imagem agora é mais rápida e eficiente. (TGT-32897)
* Aprimoramos a manipulação de redirecionamentos para URLs ao cancelar o carregamento de uma página no VEC. (TGT-33815)
...
* Após selecionar uma coleção de Recomendações no seletor de Coleções, clique no botão Salvar. Esse fluxo de trabalho é consistente com outros workflows dentro do Público alvo. (TGT-33205)
...
Esta versão do Cloud Manager (2019.3.0) deste mês adiciona o relatórios SLA ao recurso de monitoramento do sistema fornecido na última versão. Uma atualização visual de algumas telas principais também foi adicionada.
