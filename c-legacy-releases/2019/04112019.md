...
| Atualizações do Advertising Analytics | Estamos lançando algumas atualizações no Advertising Analytics: <ul><li>O Yahoo Gemini foi absorvido pelo Microsoft Bing em 31 de março. Como consequência, a opção de conta de publicidade do Yahoo Gemini não está mais disponível. </li><li>O Google implementou um novo padrão de rastreamento e, em vez de usar _Modelo de rastreamento_, agora usamos _Sufixo de URL final_.</li></ul> |
...
* Correção de um problema que ocorria ao tentar alternar segmentos em reportlets do painel. (AN-177056)
* (Reports &amp; Analytics) Correção de um problema que impedia a seleção de métricas no relatório Visão geral do canal. (AN-176786)
* (Gerenciador de segmentos) Correção de um problema que resultava em uma mensagem de erro ao tentar editar um segmento compartilhado com várias fontes de dados de Audience Manager. (AN-175353)
...
* (Administrador) Correção de um problema ao tentar editar eVars para vários conjuntos de relatórios. (AN-168150)
* (Administrador) Correção de um problema que impedia a transferência de ativos para outro usuário, na interface do usuário herdada de gerenciamento de usuários. (AN-176630, AN-173974)
