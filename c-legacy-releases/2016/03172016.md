...
| Recurso | Descrição |
|--- |--- |
| Rastreamento de falhas do aplicativo | O  iOS  SDK versão 4.8.6 contém alterações críticas que impedem que sejam relatadas falhas falsas. Recomendamos que você atualize para a versão 4.8.6. |
...
| Recurso | Descrição |
|--- |--- |
| Suporte para recusa | O serviço da Experience Cloud ID é compatível com solicitações de cancelamento. (AAM-21725) |
...
Revisamos os procedimentos de implementação do serviço de ID para os clientes do [!DNL  Analytics]. Consulte [Configurar o serviço de ID de Marketing Cloud com o Analytics](https://docs.adobe.com/content/help/pt-BR/id-service/using/home.html).
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_7CC12EBCEBD943B1B329594D13DCF66C"> 
      <li id="li_294227DF6E1346F18F5FA38805A5C2C3">Correção de um problema que impedia a criação ou edição de regras de processamento. (AN-118939) </li> 
      <li id="li_81ED9C850197465BB2E8ACE1AC9C59E6">Correção de um problema em que a senha não podia ser redefinida para nomes de usuário com mais de 20 caracteres. (AN-115603) </li> 
      <li id="li_A477D5A9DEAE4B4092990612AB2A0A59">Correção de um problema com a Análise de contribuição que fazia com que os rótulos do painel de análise fossem duplicados. (AN-118627) </li> 
      <li id="li_11840BC1D1A74248A1AD9E9B509BA5BD">Correção de um problema com reportlets de painel que não exibiam dados quando a granularidade diária era selecionada. (AN-117755) </li> 
      <li id="li_680B0AA9884B450B9E88CA9885C698FD">Corrigido um problema com a configuração do separador Milhares que revertia para o formato dos EUA (de um formato alemão). (AN-117894) </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_018BD45725944884BBF0B4C61116F5EF"> 
      <li id="li_12A2EED797D440649E4C4CC89434CC84">Correção de um problema que ocorria ao arrastar e soltar componentes um após o outro muito rapidamente. Esse problema fazia com que os indicadores de queda preta persistissem na tabela se você arrastasse um componente enquanto outra solicitação ainda estava sendo processada. (AN-118836) </li> 
      ...
      <li id="li_9AFA60D53A8F42FD8392E3C4E2D25F29">Corrigido um problema que fazia com que as visualizações redefinissem sua posição quando um projeto era atualizado, em raras circunstâncias. (AN-115865) </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
* Correção de um bug que fazia com que os relatórios de Tendência exibissem resultados por dia de folga a partir da data atual. Afeta apenas os fusos horários da Europa Oriental. (AAM-22321)
* Correção de um bug no Audience Marketplace que impedia os provedores de dados de ativarem um feed de dados privado e sem marca. (AAM-23377)
* Alterada a opção de tipo de ID no fluxo de trabalho de criação da fonte de dados para &quot;ID de anúncio do dispositivo&quot; (de &quot;Mobile&quot;). (AAM-22947)
...
* Correção de um problema de interface do usuário para que as publicações em feeds de moderação com links longos contenham barras de rolagem horizontais.
* Correção de um problema que impedia que algumas publicações fossem exibidas nos feeds de moderação de escalonamento.
...
|--- |--- |
| Formulário do AEM Assets | Nosso novo subfórum da AEM Assets está agora ao vivo. Este fórum abrange: Ativos (no local), Ativos sob demanda, Portal de marca, Mídia dinâmica e Scene7. Qualquer pessoa pode fazer logon e publicar, basta fazer logon com seu Adobe ID. |


| Conferência | Descrição |
|--- |--- |
| Adobe Immerse | O Immerse é uma conferência virtual de desenvolvedores do Adobe Experience Manager, que ocorrerá entre os dias 25 e 29 de abril. Esse evento é “obrigatório” para qualquer pessoa responsável por criar, desenvolver, implantar ou fazer a manutenção das implementações do Adobe Experience Manager.  Para obter ingressos e detalhes, consulte o site Adobe Immerse. |
