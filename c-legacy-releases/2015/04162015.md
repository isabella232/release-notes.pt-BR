...
<table id="table_04854D12F4C241AE86F24C12C4F0FA28"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>A funcionalidade de gerenciamento de usuários e grupos foi movida para o Painel Enterprise. O novo caminho de navegação é: </p> <p> <span class="uicontrol"> Experience Cloud</span> &gt; <span class="uicontrol">Ferramentas</span> &gt; <span class="uicontrol">Iniciar Painel corporativo </span></p> <p> Além disso, há suporte para Enterprise IDs e Federated IDs. Você pode usar Enterprise IDs, Federated IDs, bem como Adobe IDs na mesma implantação corporativa. Por exemplo, utilize as Adobe IDs para usuários que podem usar outros produtos e serviços da Adobe. Use Enterprise ID ou Federated IDs para usuários nos quais você deseja gerenciar estritamente as contas. </p> <p>Consulte <a href="https://docs.adobe.com/content/help/pt-BR/core-services/interface/manage-users-and-products/admin-getting-started.html" format="https" scope="external">Usuários e grupos - Gerenciamento de identidade </a>. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* Usar a ferramenta [!UICONTROL Fornecer feedback] para enviar problemas do tipo *`Bug`* agora envia esses problemas diretamente para o Atendimento ao cliente e cria um ticket. O Atendimento ao cliente deverá entrar em contato com você logo em seguida.
* Ao criar uma mensagem no aplicativo, você pode público alvo a mensagem usando *`Lifetime Value`* ou *`Locale`*.
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> Data warehouse <p> 
     <!--AN-100995--></p> </td> 
   <td colname="col2"> <p>Redução do máximo de linhas por ocorrência de 1000 para 100. </p> <p>Quando uma solicitação de data warehouse é enviada com várias dimensões de vários valores (por exemplo, vários Relatórios móveis), um número exponencial de linhas pode ser gerado a partir de uma única ocorrência. Agora estamos limitando o número de linhas que podem ser geradas a partir de uma única ocorrência em 100 (anteriormente 1.000). </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* Correção de um problema com as configurações de comparação do intervalo de datas do painel que deve ser redefinido manualmente. Observe que esta correção funciona apenas para novos marcadores; as existentes terão de ser abertas e *`saved as`*.
...
* O recurso Aplicativos sociais foi substituído oficialmente na versão Adobe Social 15.4.1.0 (abril de 2015). Isso inclui a remoção de métricas, dados e métodos de API relativos ao aplicativo. Em caso de dúvidas, entre em contato com o Gerente de contas sociais. Para obter detalhes completos, consulte Aplicativos [Adobe Social - Folha](https://helpx.adobe.com/social/kb/social-applications---deprecation-fact.html)de fatos de desaprovação.
...
* Aprimoramento da funcionalidade de upload de fotos de capa do Facebook, especialmente se ocorrer uma falha.
* Correção de um problema que causava discrepâncias pós-tempo entre publicações secundárias e principais ao duplicar publicações.
* Correção de um problema que resultava em falha de postagens devido a erros de &quot;Conflito de isolamento de transação detectado&quot;.
* Correção de um problema que impedia a exibição de imagens em miniatura em postagens publicadas em páginas do LinkedIn usando um anexo de link.
* Correção de um problema que resultava em falha de URLs encurtados publicados em páginas do LinkedIn.
