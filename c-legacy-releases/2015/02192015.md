...
<table id="table_4ABE0F980A704B069BA9044E7F32ECFE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="wintitle"> Regras </span> de classificação Analytics - substituir valores existentes </p> </td> 
...
<table id="table_EB3FFBA2DF904546A5185EC9A63BBA98"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Caso tenha várias soluções e empresas, poderá mapear vários produtos e serviços para uma única empresa ao vincular contas da solução na Experience Cloud. </p> <p>Consulte <a href="https://marketing.adobe.com/resources/help/pt_BR/mcloud/organizations.html" format="https" scope="external">Contas e empresas </a>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Added the topic <a href="https://marketing.adobe.com/resources/help/pt_BR/mcloud/core_services.html" format="https" scope="external"> Core Services - Enabling Your Solutions </a> to assist you with implementing core services. </p> </td> 
  </tr> 
 </tbody> 
</table>

Consulte a documentação do produto da [Marketing Cloud e dos principais serviços](https://docs.adobe.com/content/help/pt-BR/core-services/interface/about-core-services/core-services-landing.html) para obter mais informações.
  ...
  <tr> 
   ...
   <td colname="col2"> 
    <ul id="ul_9F4F40B4329443AC88F07B74660407C3"> 
     ...
     <li id="li_8EEB2C69113C43349F2B5E3AD9352B7B">Adicionada a capacidade de executar um conjunto de métricas sem salvá-las como padrão. Esse recurso agora está mais consistente com os relatórios padrão do Reports &amp; Analytics. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="wintitle"> Regras </span> de classificação Analytics - substituir valores existentes </p> </td> 
   <td colname="col2"> <p>Em <span class="uicontrol">Analytics</span> &gt; <span class="uicontrol">Admin</span> &gt; <span class="uicontrol">Construtor de regras de classificação</span> &gt; <span class="term"> &amp;lt;nome do conjunto de regras&amp;gt;</span>, duas novas opções permitem selecionar um modo de substituição: </p> 
    <ul id="ul_54AAEB16B74B4F53BE3549A7992B7353"> 
     ...
     <li id="li_6F71992C30E44B6FAC8E7C20D4C9D6DF"> <b>As regras substituem apenas valores não definidos:</b> preencher apenas células em branco (não definidas). As classificações existentes não serão alteradas. </li> 
    </ul> <p>Anteriormente, se uma chave já estivesse classificada em qualquer coluna ou célula, a regra não era executada nessa chave e a linha na tabela era ignorada. </p> <p>Consulte <a href="https://marketing.adobe.com/resources/help/en_US/reference/classification_rule_set.html" format="https" scope="external">Regras de classificação</a> na documentação do produto para mais informações. </p> </td> 
  </tr> 
  <tr> 
...
* Atualizações de Instância entre para segmentos sequenciais. Originalmente, esse recurso permitia que você correspondesse a um evento em um segmento sequencial, como o final de uma visita, e definisse o tempo dentro ou após a lógica com base nesses pontos de verificação do evento. Agora, uma nova função de restrição de container permite que você especifique um limite e uma contagem e identifique o container na criação de um segmento, permitindo que você crie segmentos sequenciais mais focados e complexos.
* Atualizações de Dimensão entre para segmentos sequenciais. Assim como a lógica Instância entre que identifica correspondências entre container, a Dimensão entre agora inclui uma função de restrição de dimensão que fornece lógica com base em valores de dimensão, determinando correspondências com base em valores específicos (linhas de tabela) para rastrear se uma dimensão selecionada está definida (ou não) e permitir que você forneça a lógica de segmento com base nesses valores de dimensão.
* A segmentação sequencial adiciona correspondência de várias instâncias. A implementação anterior da lógica Instance Between empregava somente a primeira correspondência de instância para segmentos sequenciais, falhando em corresponder às sequências legítimas encontradas posteriormente na visita. A nova lógica agora permite que qualquer sequência correspondente na visita seja capturada como parte do segmento sequencial. Por exemplo, um segmento sequencial que captura visitantes que foram para a *página A e depois para a página B, mas não para a página C* , não corresponderia se o visitante fosse para *A a B a C e voltasse para B*, o que corresponde em uma segunda instância (voltar para a página B corresponde à lógica *A a B e não C*).
...
>[!NOTE]
>
>O acesso do data warehouse da Versão 14 será removido nesta versão.[Saiba mais](https://docs.adobe.com/content/help/pt-BR/analytics/components/segmentation/seg-home.html)

### Feeds de dados de sequência de cliques {#clickstream}

Preenchimentos retroativos de dados para feeds de dados por hora:

Se você solicitar dados para datas anteriores ao configurar um novo feed de dados por hora, os dados para datas anteriores a mais de 60 dias podem ser entregues no formato diário em vez de por hora.

Nesse caso, você não receberá 24 delivery separados para esses dias, em vez disso, você receberá um único delivery com um carimbo de data e hora da meia-noite que contém todos os dados do dia. Se você estiver solicitando esse tipo de preenchimento retroativo, verifique se o ETL está configurado para processar delivery diários.

See [Configuring Data Feeds](https://marketing.adobe.com/resources/help/pt_BR/sc/clickstream/datafeeds_configure.html) for more information.
...
Refer to the [Adobe Target Release Notes](https://docs.adobe.com/content/help/pt-BR/target/using/release-notes/release-notes.html) for the latest dates and release information.
