...
* Audience Analytics: foi corrigido um problema que ocorria depois que um nome de público-alvo era alterado no Audience Manager (AAM). O nome atualizado não era refletido no Audience Analytics. (AN-176237)
* Correção de um problema que impedia usuários de salvar segmentos do Analytics no AAM. Isso era causado por pastas AAM existentes com nomes mistos de letras maiúsculas e minúsculas. Agora, todas as pastas diferenciam maiúsculas de minúsculas, para que sejam sincronizadas. (AN-177934)
* Correção de um problema que fazia com que a sessão expirasse logo em seguida que usuários fizessem logon no Google Analytics por meio da Experience Cloud. Ao retomar a sessão, o usuário era redirecionado para um URL com defeito. (AN-176812)
...
As Notas de versão referentes às edições de Código aberto e de Comércio podem ser encontradas na página [Informações da versão](https://devdocs.magento.com/guides/v2.3/release-notes/bk-release-notes.html).
