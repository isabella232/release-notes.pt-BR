...
[!UICONTROL Versões do appmeasurement] 2.16.0 em 15 de agosto de 2019 (não em 8 de agosto, como indicado anteriormente).
...
* (Anunciantes com o serviço de rastreamento de conversão da Marketing Cloud) Com a Apple Tracking Prevention (ITP) 2.2, que foi lançada em maio, os cookies de rastreamento de conversão da Advertising Cloud são excluídos automaticamente dos navegadores Apple Safari após 24 horas. A Advertising Cloud tem uma nova solução ITP, no entanto, permite rastrear conversões que ocorrem no Safari por mais de 24 horas após o clique original. A solução usa armazenamento local e tecnologia iframe. Entre em contato com o gerente da sua conta de pesquisa da Marketing Cloud para obter instruções de implementação.
* Em Pesquisar &gt; Avançado (ACM), agora é possível configurar os sufixos finais de URL de nível de campanha para modelos de anúncios de texto e anúncios de compra do Google.
* Os anunciantes com contas do Google Ads qualificadas para a Correspondência do cliente agora podem fazer o seguinte:
   * Crie um público-alvo de clientes do Google Ads usando IDs de usuário de um segmento de público-alvo da Adobe. Para ver esse recurso, a conta do anunciante deve ser configurada para permitir isso.
   * Crie um público-alvo do Google Ads com o upload de um arquivo de dados do cliente. O arquivo pode consistir em informações de contato (endereços de email, endereços de endereçamento ou números de telefone), IDs de usuário ou IDs de dispositivo móvel. Alguns tipos de informações de contato devem ser mascarados usando o algoritmo SHA -256.
   * Atualize qualquer público-alvo de correspondência de clientes do Google, exceto os públicos-alvo criados de um público-alvo da Adobe. Você pode carregar dados para adicionar, excluir ou substituir todos os dados existentes para o público-alvo. Todas as informações de contato devem ser hash usando o algoritmo SHA -256.
* As exibições Públicos-alvo &gt; Metas e Públicos-alvo &gt; Exclusões incluem uma coluna «Tipo».
